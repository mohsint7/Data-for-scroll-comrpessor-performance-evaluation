...
def Compressor(Te = 273, Tc = 300,Tamb = 25, f = None, OneCycle = False, Ref = 'R410A'):
    ...
    scale_JB=7
    OrbitingRadius=0.00565
    do=0.188
    
    print(scale_JB)
    ScrollComp.mech.D_upper_bearing = scale_JB*OrbitingRadius
    ScrollComp.mech.L_upper_bearing = scale_JB*OrbitingRadius
    ScrollComp.mech.c_upper_bearing = 20e-6
    ScrollComp.mech.D_crank_bearing = scale_JB*OrbitingRadius
    ScrollComp.mech.L_crank_bearing = scale_JB*OrbitingRadius
    ScrollComp.mech.c_crank_bearing = 20e-6
    ScrollComp.mech.D_lower_bearing = scale_JB*OrbitingRadius
    ScrollComp.mech.L_lower_bearing = scale_JB*OrbitingRadius
    ...
    X_d=(-0.0021*PR**3)+(0.0239*PR**2)-(0.1078*PR)+1.171
    #pc=4500
...
if __name__=='__main__':
     ScrollComp=Compressor(Te = 10, Tc =65 ,Tamb = 35, f = None, OneCycle = False, Ref = 'R410A')
    ...
    # df1.to_excel(excel_writer = "C:\Users\Mohsin\OneDrive - Oklahoma A and M System\Documents\Phd\Scroll comp/test1.xlsx", sheet_name='Sheet1',startcol=3)  
...
if __name__=='__main__':
    ...
    for i in range(len(Te)):
        for j in range(len(Tc)):
            if Te[i]==-20 and (Tc[j]==54 or Tc[j]==68):
                Wdot[i,j]=np.NaN
                mdot[i,j]=np.NaN
...
df.to_excel(excel_writer = "\',startcol=3)  
df1.to_excel(excel_writer = "\',startcol=3)  
#     
